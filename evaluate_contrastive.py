...
from ce_model import ContrastiveEmbeddingModel
...
hp_dict = {'d': 2, 'flip_prob': 0.1, 'hidden_dims': [256, 128]}
...
if categorical:
    model = ContrastiveEmbeddingModel(num_channels=x.shape[1], sample_size=x.shape[2], categorical=categorical,
                                      hyperparams=hp_dict)
else:
    model = ContrastiveEmbeddingModel(num_channels=1, sample_size=x.shape[1], categorical=categorical,
                                      hyperparams=hp_dict)
...
if os.path.isfile(groups_file):
    ...
    plot_scatter(embs[:, 0], embs[:, 1], labels, title='Contrastive', legend=False)
else:
    plot_scatter(embs[:, 0], embs[:, 1], title='Contrastive')

plt.savefig('contrastive.png')
plt.savefig('contrastive.pdf')

dm = cdist(embs, embs, 'euclidean')
dump(dm, 'contrastive_distance_matrix.bin')

if do_log:
    wandb.log({"plot": wandb.Image("contrastive.png")})
...
writer(open(os.path.join(os.path.dirname(data_file), 'contrastive_embs.csv'), 'w+', newline='')).writerows(tosave)
